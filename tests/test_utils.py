...
@pytest.mark.parametrize(
    "filter_cls",
    [FakeDataClassFilter, FakeBlogFilter, FakeBlogPydanticFilter],
)
def test_filter_variants(filter_cls):
    def endpoint(
        _: filter_cls = Depends(filter_cls),
    ):
        pass

    sig = inspect.signature(endpoint)
    assert get_signature_func(sig) == {
        "blog_id": {"alias": "id", "multiple": False},
        "slug": {"alias": None, "multiple": False},
    }


def test_ignore_non_filter_depends():
    class DummySession:
        pass

    def get_dummy_session() -> DummySession:
        return DummySession()

    def endpoint(
        db_session: DummySession = Depends(get_dummy_session),
...
def test_old_depends_in_signature():
    def endpoint(
        filter_: FakeBlogPydanticFilter = Depends(),
...
def test_signature_func_direct_alias():
    ...
    sig = inspect.signature(endpoint)
    assert get_signature_func(sig) == {"q": {"alias": "query", "multiple": False}}


def test_query_models_extract_params():
    def endpoint(filter_: Annotated[FakeBlogPydanticFilter, Query()]):
        return
