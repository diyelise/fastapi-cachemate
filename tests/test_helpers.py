...
from fastapi_cachemate.core.helpers import _pack_payload, _unpack_payload, load_from_cache, save_to_cache
...
def test_pack_payload_adds_marker(compressed, marker):
    data = b"payload"
    assert _pack_payload(data, compressed=compressed) == marker + data


@pytest.mark.parametrize(
    ("raw", "expected"),
    [
        (b"\x01payload", (True, b"payload")),
        (b"\x00payload", (False, b"payload")),
    ],
)
def test_unpack_payload_known_markers(raw, expected):
    assert _unpack_payload(raw) == expected


@pytest.mark.parametrize("raw", [b"", b"\x02payload"])
def test_unpack_payload_unknown_marker_returns_none(raw):
    assert _unpack_payload(raw) is None
